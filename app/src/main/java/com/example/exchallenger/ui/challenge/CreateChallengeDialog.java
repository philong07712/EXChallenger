...
public class CreateChallengeDialog extends DialogFragment {
    ...
    List<String> types = Arrays.asList("Push up", "Plank", "Squat");
    ...
    public static CreateChallengeDialog newInstance(ChallengeItem challengeItem, int pos,
                                                    OnSubmitListener onSubmitListener) {
        CreateChallengeDialog dialog = new CreateChallengeDialog();
        dialog.setChallengeItem(challengeItem);
        dialog.setPos(pos);
    ...
    public void setUp() {
        ...
        if (challengeItem != null) {
            selectTypePos = Math.max(0, types.indexOf(challengeItem.getType()));
            updateSeekBar();
            timePicker.setCurrentHour(challengeItem.getHour());
            timePicker.setCurrentMinute(challengeItem.getMinute());
            boolean isRepeat = AppUtils.isRepeat(challengeItem.getRepeat());
            switchRepeat.setChecked(isRepeat);
            if (isRepeat) {
                viewPickDate.setVisibility(View.VISIBLE);
                tvSunday.setSelected(challengeItem.getRepeat().get(0) == 1L);
                tvMonday.setSelected(challengeItem.getRepeat().get(1) == 1L);
                tvTuesday.setSelected(challengeItem.getRepeat().get(2) == 1L);
                tvWed.setSelected(challengeItem.getRepeat().get(3) == 1L);
                tvThur.setSelected(challengeItem.getRepeat().get(4) == 1L);
                tvFri.setSelected(challengeItem.getRepeat().get(5) == 1L);
                tvSat.setSelected(challengeItem.getRepeat().get(6) == 1L);
            } else {
                viewPickDate.setVisibility(View.GONE);
            }
        }
    ...
    public void onBtnOkClicked() {
        ...
        ChallengeItem _challengeItem = new ChallengeItem(type, seekBarNumber.getProgress(),
        ...
        if (onSubmitListener != null) {
            onSubmitListener.onSubmit(pos, _challengeItem);
        }
        dismiss();
    }
    ...
    public interface OnSubmitListener {
        void onSubmit(int pos, ChallengeItem challengeItem);
    }
}
